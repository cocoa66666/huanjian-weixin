		...
		<scroll-view :scroll-top="scrollTop" scroll-y="true" class="scroll-Y" @scroll="scroll">
			...
			<view class="price">
				<view class="price_table">
					...
					<view class="price_content" style="border-left: none;border-right: none;">服务费（元）</view>
					<view class="price_content">电费（元）</view>
				</view>
				<block v-for="(item,index) in listTimeSlot" :key="index">
					<view class="price_table2">
						<view class="price_content2">{{item.timeSlot}}</view>
						<view class="price_content2" style="border-left: none;border-right: none;">{{item.serviceFee}}</view>
						<view class="price_content2">{{item.electricityFee}}</view>
					</view>
				</block>
			</view>
			...
			<view class="dc_pile">
				<view class="dc_pile_sub" data-index="0" :class="plie_active==0?'dc_pile_sub_word':''" @tap="changePile">
					<span style='background-color: #FEA503;padding: 12rpx;border-radius: 50rpx;'>
						<span style='color: #ffffff;background-color: #4FB773;border-radius: 50rpx;padding: 12rpx;margin-left: -12rpx;'>{{pileFree}}空闲</span>
			...
			<view class="dc_content" v-if="plie_active==0">
				<view class="dc_content_sub" v-for="(item, index) in pileInfo" :key="index">
					...
					<view v-if="item.status==2||item.status==3||item.status==4" class="dc_content_img14">占用</view>
